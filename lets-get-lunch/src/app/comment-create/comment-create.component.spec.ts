...
describe('CommentCreateComponent', () => {
  ...
  it('should display an error message if no comments exist for the event', () => {
    ...
    expect(noComments.textContent.trim()).toEqual('No comments exist for this event.');
  })

  it('should update the list of comments when a user submits a comment', () => {
    const strContent = 'My first comment';
    ...
    fixture.debugElement.query(By.css('textarea.form-control')).nativeElement.value = strContent;
    fixture.debugElement.query(By.css('button.btn.btn-primary')).nativeElement.click();
    fixture.detectChanges();
    ...
    expect(comments.pop().content).toEqual(strContent);

    const commentList = fixture.debugElement.queryAll(By.css('.well.well-sm'));
    expect(commentList[0].nativeElement.textContent).toContain(strContent);
  })

  it('should show an error message if the event cannot be created', async () => {
    spyOn(commentsService, 'create').and.callFake(() => {
    ...
    fixture.debugElement.query(By.css('textarea')).nativeElement.value = 'This is an erroneous post';
    fixture.debugElement.query(By.css('button')).nativeElement.click();

    expect(commentsService.create).toHaveBeenCalled();
    expect(commentsService.getEventComments).toHaveBeenCalledTimes(1);

    const errorMsg = fixture.debugElement.query(By.css('[data-test=submit-error]'))
      .nativeElement;
    expect(errorMsg.textContent).toEqual('Comment could not be created!');
  })
})