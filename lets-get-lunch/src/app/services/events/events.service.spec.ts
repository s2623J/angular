...
describe('EventsService', () => {
  ...
  describe('all', () => {
    ...
    it('should return an error if there is a server error', () => {
      const error = 'Something went wrong!';
      let errorResponse;

      eventsService.all().subscribe(() => {}, err => {
        errorResponse = err;
      })
  ...
  describe('isEventCreator', () => {
    it('should return true if the event creator is the current user', () => {
      let id = '58dab4f21342131b8c96787f';
      expect(eventsService.isEventCreator(id)).toBe(true);
    })

    it('should return false if the event creator is not the current user', () => {
      let id = 'fakeId';
      expect(eventsService.isEventCreator(id)).toBe(false);
    })
  })
  ...
  describe('subscribe', () => {
    ...
    it('should return an error if a user cannot be subscribed to an event', () => {
      ...
      eventsService.subscribe(eventId, subscriber).subscribe(() => {}, err => {
        errorResponse = err;
      })
      http
        .expectOne('http://localhost:8080/api/events/' + eventId + '/subscribe')
  ...
  describe('update', () => {
    it('should return an updated event with valid event details', () => {
      ...
      http
        .expectOne("http://localhost:8080/api/events/5a55135639fbc4ca3ee0ce5a")
