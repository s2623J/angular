/// <reference types="cypress", "jasmine"/>
describe('Event Subscribe', () => {
  ...
  it('should not display a subscribe button for the event creator', () => {
    cy
      .login(creator, 'foobar')
      .get('a.cal-event-title').should('have.text', eventName).click()
      .url().should('contain', '/event/')
      .get('.event-subscribe').should('not.exist');
  })

  it('should update the member list when a user clicks both the subscribe and ' + 
    'unsubscribe button', () => {
    cy
      .login(subscriber, 'foobar')
      .get('[data-test=events]').click()
      .url().should('contain', '/events')
      .get('td.event-title').should('contain.text', 'MyEvent').next().next()
        .children('a').should('contain.text', 'View').click()
      
      .url().should('contain', '/event/')
      .get('[data-test=subscribe]').should('contain.text', 'Subscribe').click()
